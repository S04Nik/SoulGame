...
namespace DarkSoul
{
    public class EquipmentWindowUI : MonoBehaviour
    {
        ...
        private void Start()
        {
            // doesnt work if not enabled component
            handEquipmentSlotUI = GetComponentsInChildren<HandEquipmentSlotUI>();
        }

        public void LoadWeaponOnEquipmentScreen(PlayerInventory inventory)
        {
            // handEquipmentSlotUI empty
            for (int i = 0; i < handEquipmentSlotUI.Length; i++)
            {
                ...
                {
                    handEquipmentSlotUI[i].AddItem(inventory.weaponsInRightHandSlots[2]);
                }
                else if (handEquipmentSlotUI[i].rightHandSlot04)
                {
                    handEquipmentSlotUI[i].AddItem(inventory.weaponsInRightHandSlots[3]);
                }
                ...
                {
                    handEquipmentSlotUI[i].AddItem(inventory.weaponsInLeftHandSlots[2]);
                }
                else if (handEquipmentSlotUI[i].leftHandSlot04)
                {
                    handEquipmentSlotUI[i].AddItem(inventory.weaponsInLeftHandSlots[3]);
                }
            }
        }
